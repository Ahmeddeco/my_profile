...
import { addBranchAction } from "@/app/[locale]/server/courses/branches/modules/branch.action"
...
export default function AddBranch({ academies }: Props) {
	const [slug, setSlug] = useState("")
	const slugTitle = customSlug(slug)
	const [lastResult, action] = useActionState(addBranchAction, undefined)
	...
	return (
		<Form id={form.id} action={action} onSubmit={form.onSubmit} className="space-y-6">
			...
			<Field>
				...
				<Select key={fields.academyId.key} name={fields.academyId.name} defaultValue={fields.academyId.initialValue}>
					<SelectTrigger>
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
